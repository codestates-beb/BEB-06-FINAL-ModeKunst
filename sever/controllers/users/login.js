        ...
        if (user && comparePassword) {
          ...
          if (!(parseInt(user.login_at) === 0)) {
            ...
            if ((now - parseInt(user.dataValues.login_at) >= 86400000)) {
              ...
              const { login } = await Token_price.findOne({ where: { id: 1 }, attributes: ['login'], raw: true });
              ...
              const data = contract.methods.transfer(user.address, login).encodeABI();
              ...
              if (sendSignTx) {
                ...
                await User.update({ token_amount: clientBalance, login_at: now  }, { where: { email: email } });
