    ...
    post: async (req, res) => {
        ...
        try {
            ...
            if(!review){
                // 여기가 새 리뷰
                try{
                    const price = await Token_price.findOne({ where: { id: 1 }, attributes: ['review'], raw: true });
                    ...
                    const data = contract.methods.transfer(userAddress, review_price).encodeABI();
                    ...
                    await Server.update({ eth_amount: server_eth, token_amount: serverBalance }, { where: { address: address } });

                    await User.update({ token_amount: clientBalance }, { where: { nickname: nickname } });
                    ...
                    await Post.update({ reviews_num: review_counts }, {where: { id: postId } })
