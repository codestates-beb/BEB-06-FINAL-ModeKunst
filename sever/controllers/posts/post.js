  ...
  post: async (req, res) => {
    ...
    if (req.files.length <= 2) {
      ...
      if (top_post) {
        ...
        try {
          const { point_amount } = await User.findOne({
            where: { nickname: nickname },
            attributes: ["point_amount"],
            raw: true,
          });
              ...
              if (write_count !== 0) {
                if (haveInfo) {
                  ...
                  await Server.increment(
                    {
                      point_amount: write_post_info,
                      used_point: top_post_price,
                    },
                  ...
                  res.status(200).json({
                    message:
                      "게시물이 등록 되었습니다. ( 10 Mode Point를 받았습니다! )",
                    data: {
                  ...
                  await Server.increment(
                    { point_amount: write_post, used_point: top_post_price },
                    { where: { address: address } }
                  );

                  await User.increment(
                    { point_amount: write_post },
                    { where: { nickname: nickname } }
                  );
                  ...
                  res.status(200).json({
                    message:
                      "게시물이 등록 되었습니다. ( 5 Mode Point를 받았습니다! )",
                    data: {
                      postId: post.dataValues.id,
                    },
                  });
        ...
        try {
          ...
          if (write_count !== 0) {
            if (haveInfo) {
              ...
              res.status(200).json({
                message:
                  "게시물이 등록 되었습니다. 게시물이 등록 되었습니다. ( 10 Mode Point를 받았습니다! )",
                data: {
                  postId: post.dataValues.id,
                },
              });
              ...
              res.status(200).json({
                message:
                  "게시물이 등록 되었습니다. 게시물이 등록 되었습니다. ( 5 Mode Point를 받았습니다! )",
                data: {
                  postId: post.dataValues.id,
                },
              });
