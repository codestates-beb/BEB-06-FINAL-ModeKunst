...
module.exports = {
  ...
  get: async (req, res) => {
    ...
    if (notice) {
      ...
      return res
        .status(404)
        .json({ message: "해당하는 공지/래플(이)가 없습니다." });
    }
  },
  //공지 업데이트
  put: async (req, res) => {
    ...
    try {
      ...
      if (oldNotice.token_price == 0) {
        ...
        if (token_price > 0) {
          return res
            .status(404)
            .json({ message: "공지의 token_price는 수정이 불가합니다." });
        }
        ...
        fileNames.map(name => {
          if (name) {
            if (
              fs.existsSync(
                path.join(__dirname, "..", "..", "notice_img", `${name}`)
              )
            ) {
              try {
                fs.unlinkSync(
  ...
  getAllList: async (req, res) => {
    ...
    const notices = noticeList.map((el, idx) => {
      return {
        id: el.dataValues.id,
        title:
          el.dataValues.token_price > 0
            ? "[래플]" + el.dataValues.title
            : "[공지]" + el.dataValues.title,
